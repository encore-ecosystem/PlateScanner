...
def __validate_yolo_with_clearml(model_path: PathVariable, config: dict):
        ...
        iteration = 0
        task.get_logger().report_scalar(title='Validation Metrics mAP Rus', series="mAP 50", value=results.box.map50,
                                        iteration=iteration)
        task.get_logger().report_scalar(title='Validation Metrics mAP Rus', series="mAP 75", value=results.box.map75,
                                        iteration=iteration)
