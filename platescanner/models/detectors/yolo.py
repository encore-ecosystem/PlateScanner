import torch.hub
from cvtk import AbstractDataset, Bbox
...
class Yolo(Model):
    def __init__(
            ...
            weights_path: str | Path = 'ultralytics/yolov5',
            conf: float = 0,
            area_threshold: float = float('inf'),
    ):
        if download:
            self.model = torch.hub.load(
                weights_path.__str__(),
                'yolov5n',
    ...
    def predict(self, input_tensor) -> list[Bbox]:
        width, height = input_tensor.shape[0:2]

        results = self.compute(input_tensor)
